...
**Логика фильтрации по локации:**

| Настройка | Результат |
|-----------|-----------|
| `anyLocation: true` | Показываются все анкеты |
| Нет города (даже с координатами) | Показываются все анкеты |
| Город указан, координат нет | Фильтр по городу (точное совпадение) |
| Город + координаты | Фильтр по расстоянию (maxDistance км) |
...
**Важно:** Координаты без города не влияют на фильтрацию — нужен город для активации фильтра по расстоянию.
...
*По полу:*
- `lookingFor = MALE` → только мужчины
- `lookingFor = FEMALE` → только женщины
- `lookingFor = BOTH` → все
- Также проверяется обратное: анкета показывается только если её `lookingFor` включает ваш пол

*По возрасту:*
- `minAge` / `maxAge` — фильтр по дате рождения

*По локации:*

| Ваш профиль | Фильтрация |
|-------------|------------|
| `anyLocation = true` | Все анкеты, без фильтра |
| Нет города (даже с координатами) | Все анкеты, без фильтра |
| Город указан, координат нет | По городу (точное совпадение) |
| Город указан И есть координаты | По расстоянию (`distance <= maxDistance` км) |

*Исключения:*
- Пользователи с активными взаимодействиями (24ч cooldown)
- Активные матчи
- Матчи в периоде скрытия (2 дня после размэтча)
